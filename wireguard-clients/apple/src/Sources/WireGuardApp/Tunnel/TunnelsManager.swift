...
class TunnelsManager {
    ...
    private func saveConfigsFromCurrentTunnelProvider() {
    ...
    func modify(tunnel: TunnelContainer, tunnelConfiguration: TunnelConfiguration, onDemandOption: ActivateOnDemandOption, completionHandler: @escaping (TunnelsManagerError?) -> Void) {
        ...
        if tunnelProviderManager.tunnelConfiguration != tunnelConfiguration {
            tunnelProviderManager.modifyTunnelConfiguration(tunnelConfiguration)
    ...
    func activatingTunnel() -> TunnelContainer? {
        return tunnels.first { $0.status == .activating }
    }
    ...
    func turnOnActivatedTunnel(tunnelName: String) {

        if tunnelName.isEmpty {
            wg_log(.info, message: "There aren't tunnels in activating status")
            return
        }
        if let tunnel = tunnel(named: tunnelName) {
            self.startWgTunnelActivation(of: tunnel)
        } else {
            wg_log(.info, message: "There aren't tunnels with required name")
        }
    }

    func resetTunnelState(tunnelName: String) {
    ...
    func startActivation(of tunnel: TunnelContainer) {
        ...
        guard tunnels.contains(tunnel) else {
            return
        ...
        if let config = tunnel.tunnelConfiguration?.activationConfig {
            actuvationManager.getActivationData(config: config, tunnelName: tunnel.name) { [weak self] result in
                switch result {
                case .noError, .noActivationNeeded: self?.startWgTunnelActivation(of: tunnel)
                case .serverError, .httpError, .noInternet, .invalidCredentials:
                    tunnel.status = .inactive
                    self?.activationDelegate?.tunnelActivationAttemptFailed(
                            tunnel: tunnel,
                            error: .failedWhileLoading(
                                    systemError: NSError(domain: result.description,
            ...
            startWgTunnelActivation(of: tunnel)
        }
    }

    func startWgTunnelActivation(of tunnel: TunnelContainer) {
        ...
        waiteeObservationToken = tunnel.observe(\.status) { [weak self] tunnel, _ in
            ...
            if tunnel.status == .inactive {
                if let waitingTunnel = self.tunnels.first(where: { $0.status == .waiting }) {
                    if let config = waitingTunnel.tunnelConfiguration?.activationConfig {
                        self.actuvationManager.getActivationData(config: config, tunnelName: waitingTunnel.name) { [weak self] result in
                            switch result {
                            case .noError, .noActivationNeeded:  waitingTunnel.startActivation(activationDelegate: self?.activationDelegate)
                            case .serverError, .httpError, .noInternet, .invalidCredentials:
                                waitingTunnel.status = .inactive
                                self?.activationDelegate?.tunnelActivationAttemptFailed(
                                        tunnel: waitingTunnel,
                                        error: TunnelsManagerActivationAttemptError.failedWhileLoading(
                                                systemError: NSError(domain: result.description,
                                                        code: result.errorCode)
                                        )
                                )
...
extension NETunnelProviderManager {
    ...
    func modifyTunnelConfiguration(_ newTunnelConfiguration: TunnelConfiguration) {
        let mergedTunnelConfiguration = TunnelConfiguration(
                name: newTunnelConfiguration.name,
                interface: newTunnelConfiguration.interface,
                peers: newTunnelConfiguration.peers,
                activationConfig: tunnelConfiguration?.activationConfig
        )
        protocolConfiguration = NETunnelProviderProtocol(tunnelConfiguration: mergedTunnelConfiguration, previouslyFrom: protocolConfiguration)
        localizedDescription = newTunnelConfiguration.name
        objc_setAssociatedObject(self, &NETunnelProviderManager.cachedConfigKey, mergedTunnelConfiguration, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    }
