...
class TunnelListCell: UITableViewCell {
    ...
    let onColor  = ToggleCustomStyle.onColor
    let offColor = ToggleCustomStyle.offColor
 ...
 private func updateStyle() {
        if statusSwitch.isOn {
            statusSwitch.onTintColor = ToggleCustomStyle.onColor
        } else {
            /*For off state*/
            statusSwitch.tintColor = ToggleCustomStyle.offColor
            statusSwitch.layer.cornerRadius = statusSwitch.frame.height / 2.0
            statusSwitch.backgroundColor = ToggleCustomStyle.offColor
