...
extension TunnelConfiguration {
    ...
    enum ParseError: Error {
        ...
        case configHasUnrecognizedKey(String)
        case configHasNotHostKey
        case configHasNotPublicKey
    }
    ...
    /**
     ...
     [Interface]
     PrivateKey = QBAdZNPSePWCUty8XT9LhBBWRNYpbuoif8laxYahEns=
     Address = 10.201.0.3
     DNS = 8.8.8.8

     [Peer]
     PublicKey = mLaMbmZisFtfiIqFJRT0XoCwijocIs76CKWAT/AMoDM=
     Endpoint = upgradable-8-gateway.zerotrustedge.com:51820
    ...
    convenience init(fromWgQuickConfig wgQuickConfig: String, called name: String? = nil) throws {
        ...
        for (lineIndex, line) in lines.enumerated() {
            ...
            if !trimmedLine.isEmpty {
                if let equalsIndex = trimmedLine.firstIndex(of: "=") {
                    ...
                    let configSectionKeys: Set<String> = [ConfigSection.host, ConfigSection.key]
                    ...
                    } else if parserState == .inConfigSection {
                        guard configSectionKeys.contains(key) else {
                            throw ParseError.configHasUnrecognizedKey(keyWithCase)
                        }
                    }
                } else if lowercasedLine != "[interface]" && lowercasedLine != "[peer]" && lowercasedLine != ConfigSection.rootParser {
            ...
            if isLastLine || lowercasedLine == "[interface]" || lowercasedLine == "[peer]" || lowercasedLine == ConfigSection.rootParser {
                ...
                if parserState == .inInterfaceSection {
                    ...
                    let config = try TunnelConfiguration.collate(configAttributes: attributes)
                    guard activationConfig == nil else { throw ParseError.multipleInterfaces }
            ...
            if lowercasedLine == "[interface]" {
                ...
                attributes.removeAll()
            } else if lowercasedLine == ConfigSection.rootParser {
                parserState = .inConfigSection
                attributes.removeAll()
            }
        }
        ...
        if let interfaceConfiguration = interfaceConfiguration {
            self.init(name: name, interface: interfaceConfiguration, peers: peerConfigurations, activationConfig: activationConfig)
        } else {
    ...
    /**
     ...
     AllowedIPs = 0.0.0.0/0, ::/0
     */
    func asZTEConfig() -> String {
        var output = ""

        if let activationConfig = activationConfig {
            output = "[\(ConfigSection.root)]\n"
            output.append("\(ConfigSection.host) = \(activationConfig.host)\n")
            output.append("\(ConfigSection.key) = \(activationConfig.key)\n")
        }
    ...
    private static func collate(configAttributes attributes: [String: String]) throws -> ActivationConfig {
        guard let hostKeyString = attributes["host"] else {
            throw ParseError.configHasNotHostKey
        }

        guard let keyString = attributes["key"] else {
            throw ParseError.configHasNotPublicKey
        }

        return ActivationConfig(host: hostKeyString, key: keyString)
    }
}
