...
func (conf *Config) Save(overwrite bool) error {
	if !TunnelNameIsValid(conf.Name) {
		return errors.New("Tunnel name is not valid")
	}
	configFileDir, err := tunnelConfigurationsDirectory()
	if err != nil {
	...
	filename := filepath.Join(configFileDir, conf.Name+configFileSuffix)
	bytes := []byte(conf.ToZteConfig())
	bytes, err = dpapi.Encrypt(bytes, conf.Name)
	if err != nil {
		return err
	}
...
func (conf *Config) Path() (string, error) {
	if !TunnelNameIsValid(conf.Name) {
		return "", errors.New("Tunnel name is not valid")
	}
	configFileDir, err := tunnelConfigurationsDirectory()
	...
	return filepath.Join(configFileDir, conf.Name+configFileSuffix), nil
}
