...
const (
	inConfigSection parserState = iota
	inInterfaceSection
	inPeerSection
	notInASection
)
...
func (conf *Config) maybeAddActivationConfig(p *ActivationConfig) {
	if p != nil {
		conf.ActivationConfig = *p
	}
}
	...
	for _, line := range lines {
		...
		if parserState == inInterfaceSection {
			...
			case "host":
				...
				activationConfig.Host = *e
			case "key":
				key, err := parseKeyBase64(val)
				if err != nil {
					return nil, err
				}
				activationConfig.Key = *key
			default:
				return nil, &ParseError{l18n.Sprintf("Invalid key for [Config] section"), key}
			}
		}
	}
	conf.maybeAddPeer(peer)
	conf.maybeAddActivationConfig(activationConfig)
