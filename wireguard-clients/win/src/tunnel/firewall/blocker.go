...
func createWfpSession() (uintptr, error) {
	sessionDisplayData, err := createWtFwpmDisplayData0("ZTE Client", "ZTE Client dynamic session")
	if err != nil {
...
func registerBaseObjects(session uintptr) (*baseObjects, error) {
	...
	//
	{
		displayData, err := createWtFwpmDisplayData0("ZTE Client", "ZTE Client provider")
		if err != nil {
			return nil, wrapErr(err)
		}
	...
	//
	{
		displayData, err := createWtFwpmDisplayData0("ZTE Client filters", "Permissive and blocking filters")
		if err != nil {
			return nil, wrapErr(err)
		}
