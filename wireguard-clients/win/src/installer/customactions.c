...
#define MANAGER_SERVICE_NAME TEXT("ZteManager")
#define TUNNEL_SERVICE_PREFIX TEXT("ZteTunnel$")
...
static void log_messagef(MSIHANDLE installer, enum log_level level, const TCHAR *format, ...)
{
	...
	switch (level) {
	case LOG_LEVEL_INFO:
		template = TEXT("ZTEdge Client: [1]");
		type = INSTALLMESSAGE_INFO;
		break;
	case LOG_LEVEL_WARN:
		template = TEXT("ZTEdge Client warning: [1]");
		type = INSTALLMESSAGE_INFO;
		break;
	case LOG_LEVEL_ERR:
		template = TEXT("ZTEdge Client error: [1]");
...
__declspec(dllexport) UINT __stdcall CheckWow64(MSIHANDLE installer)
{
	...
	log_messagef(installer, LOG_LEVEL_MSIERR, TEXT("You must use the native version of ZTEdge Client on this computer."));
...
static int message_box(MSIHANDLE installer, TCHAR *text, UINT type)
{
	...
	if (MsiGetProperty(installer, TEXT("MsiUIProgressOnly"), progressOnly, &len) == ERROR_SUCCESS && _tcstoul(progressOnly, NULL, 10) == 1)
		return MessageBox(GetForegroundWindow(), text, TEXT("ZTEdge Client"), type);
...
__declspec(dllexport) UINT __stdcall CheckKB2921916(MSIHANDLE installer)
{
	...
	if (MsiGetProperty(installer, TEXT("UILevel"), uiLevel, &len) != ERROR_SUCCESS || _tcstoul(uiLevel, NULL, 10) < INSTALLUILEVEL_BASIC) {
		log_messagef(installer, LOG_LEVEL_MSIERR, TEXT("Use of ZTEdge Client on Windows 7 requires KB2921916."));
		goto out;
	}
#ifdef _WIN64
...
#endif
	if (message_box(installer, TEXT("Missing Windows Hotfix\n\nUse of ZTEdge Client on Windows 7 requires KB2921916. Would you like to download the hotfix in your web browser?"), MB_ICONWARNING | MB_YESNO) == IDYES)
...
__declspec(dllexport) UINT __stdcall LaunchApplicationAndAbort(MSIHANDLE installer)
{
	...
	if (!path[0] || !PathAppend(path, TEXT("zte.exe")))
		goto out;
	log_messagef(installer, LOG_LEVEL_INFO, TEXT("Launching %1"), path);
	if (!CreateProcess(path, TEXT("zte"), NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
...
__declspec(dllexport) UINT __stdcall RemoveConfigFolder(MSIHANDLE installer)
{
	...
	RegDeleteKey(HKEY_LOCAL_MACHINE, TEXT("Software\\ZTE")); // Assumes no WOW.
out:
	if (is_com_initialized)
		CoUninitialize();
	return ERROR_SUCCESS;
