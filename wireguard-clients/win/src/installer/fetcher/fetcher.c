...
static void set_status(HWND progress, const char *status)
{
	...
	char buf[0x1000];
	g_total = 0;
	_snprintf_s(buf, sizeof(buf), _TRUNCATE, "ZTEdge Client: %s...", status);
...
out:
	...
	if (ret) {
		...
		if (MessageBoxA(progress, "Something went wrong when downloading the ZTEdge Client installer. Would you like to open your web browser to the MSI download page?", "Download Error", MB_YESNO | MB_ICONWARNING) == IDYES)
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR pCmdLine, int nCmdShow)
{
	...
	progress = CreateWindowExA(0, PROGRESS_CLASS, "ZTEdge Client Installer",
