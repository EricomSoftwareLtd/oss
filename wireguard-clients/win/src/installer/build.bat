...
:build
	...
	echo [+] Signing
	signtool sign /sha1 "%SigningCertificate%" /fd sha256 /tr "%TimestampServer%" /td sha256 /d "ZTEdge Client Setup" "dist\ericom_zte_client-*-%WIREGUARD_VERSION%.msi" || goto :error
...
:msi
	...
	echo [+] Linking %1
	"%WIX%bin\light" %WIX_LIGHT_FLAGS% -out "dist\ZTEdge-%~1-%WIREGUARD_VERSION%.msi" "%~1\wireguard.wixobj" || exit /b %errorlevel%
	goto :eof
