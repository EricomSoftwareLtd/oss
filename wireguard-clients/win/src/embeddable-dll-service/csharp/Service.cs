...
namespace Tunnel
{
    ...
    {
        private const string LongName = "Example ZTEdge Client Tunnel Client";
        private const string Description = "A ZTEdge Client tunnel created by example code.";

        [DllImport("tunnel.dll", EntryPoint = "ZteTunnelService", CallingConvention = CallingConvention.Cdecl)]
        ...
        {
            var pipepath = "ProtectedPrefix\\Administrators\\ZTE\\" + Path.GetFileNameWithoutExtension(configFile);
        ...
        public static void Add(string configFile)
        {
            ...
            var shortName = String.Format("ZteTunnel${0}", tunnelName);
