...
import (
	"fmt"
	"golang.zx2c4.com/wireguard/windows/mfa"
	"strconv"
	"strings"
	"time"
...
func newToggleActiveLine(parent walk.Container) (*toggleActiveLine, error) {
	...
	if tal.cancelButton, err = walk.NewPushButton(tal.composite); err != nil {
		return nil, err
	}
	tal.cancelButton.SetText(l18n.Sprintf("Cancel"))
	tal.cancelButton.SetVisible(false)
	disposables.Add(tal.cancelButton)
...
func NewConfView(parent walk.Container) (*ConfView, error) {
	...
	cv.interfaze.toggleActive.cancelButton.Clicked().Attach(cv.onToggleCancelClicked)
	...
	cv.cancelChannel = make(chan bool, 1)
...
func (cv *ConfView) SetEnableActivationButton(enabled bool) {
	cv.interfaze.toggleActive.button.SetEnabled(enabled)
}

func (cv *ConfView) SetVisibleCancelButton(visible bool) {
	cv.interfaze.toggleActive.cancelButton.SetVisible(visible)
}

func (cv *ConfView) onToggleCancelClicked() {
	cv.SetEnableActivationButton(true)
	cv.SetVisibleCancelButton(false)
}

func (cv *ConfView) onToggleActiveClicked() {

	cv.SetEnableActivationButton(false)

	currentTunnelState, err := cv.tunnel.State()
	if err != nil {
		cv.SetEnableActivationButton(true)
		return
	}
	shouldActivate := currentTunnelState == manager.TunnelStopped && cv.config.HasActivationConfig()

	if shouldActivate == true {

		cv.SetVisibleCancelButton(true)

		go func() {
			result := make(chan int)
			mfa.Activate(*cv.config, result)
			select {
			case <-cv.cancelChannel:
			case t := <-result:
				cv.SetVisibleCancelButton(false)
				if t != 0 {
					cv.SetEnableActivationButton(true)
					showErrorCustom(cv.Form(), l18n.Sprintf("Failed to activate tunnel"), fmt.Sprintf("Failed to activate config\n%s", mfa.FormatErrorMessage(t)))
					return
				} else {
					cv.internalTunnelActivation()
				}
			}
		}()
	} else {
...
func (cv *ConfView) internalTunnelActivation() {
	go func() {
