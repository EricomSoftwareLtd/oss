...
func runAboutDialog(owner walk.Form) error {
	...
	showingAboutDialog.SetTitle(l18n.Sprintf("About ZTEdge Client"))
	...
	iv.MouseUp().Attach(func(x, y int, button walk.MouseButton) {
		if button == walk.LeftButton {
			win.ShellExecute(showingAboutDialog.Handle(), nil, windows.StringToUTF16Ptr("https://www.ericom.com/"), nil, nil, win.SW_SHOWNORMAL)
	...
	iv.Accessibility().SetName(l18n.Sprintf("ZTEdge Client logo image"))
	...
	detailsLbl.SetText(l18n.Sprintf("Operating system: %s", version.OsName()))
	...
	copyrightLbl.SetText("Copyright @ 2021 Ericom Software. All Rights Reserved.")
