...
:sign
	...
	echo [+] Signing
	signtool sign /sha1 "%SigningCertificate%" /fd sha256 /tr "%TimestampServer%" /td sha256 /d Ericom x86\ZTEdge.exe x86\ZTEdge-cli.exe amd64\ZTEdge.exe amd64\ZTEdge-cli.exe arm\ZTEdge.exe arm\ZTEdge-cli.exe arm64\ZTEdge.exe arm64\ZTEdge.exe || goto :error

:success
	echo [+] Success. Launch "ZTEdge.exe"
	exit /b 0
...
:build_plat
	...
	go build -tags load_wintun_from_rsrc -ldflags="-H windowsgui -s -w" -trimpath -v -o "%~1\ZTEdge.exe" || exit /b 1
	if not exist "%~1\ZTEdge-cli.exe" (
		...
		move /Y .deps\src\wg.exe "%~1\ZTEdge-cli.exe" > NUL || exit /b 1
	)
	goto :eof
