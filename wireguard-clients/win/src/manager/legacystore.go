...
func moveConfigsFromLegacyStore() {
	...
	oldC := filepath.Join(oldRoot, "ZTE", "Configurations")
	...
	oldLog16, err := windows.UTF16PtrFromString(filepath.Join(oldRoot, "ZTE", "log.bin"))
	if err == nil {
		windows.MoveFileEx(oldLog16, nil, windows.MOVEFILE_DELAY_UNTIL_REBOOT)
	}
	oldRoot16, err := windows.UTF16PtrFromString(filepath.Join(oldRoot, "ZTE"))
	if err == nil {
	...
	for _, svcName := range services {
		if !strings.HasPrefix(svcName, "ZteTunnel$") {
			continue
		}
