...
object UserKnobs {
    ...
    private val ACTIVATING_TUNNEL = preferencesKey<String>("activating_tunnel")
    val activatingTunnel: Flow<String?>
        get() = Application.getPreferencesDataStore().data.map {
            it[ACTIVATING_TUNNEL]
        }

    suspend fun setActivatingTunnel(activatingTunnel: String?) {
        Application.getPreferencesDataStore().edit {
            if (activatingTunnel == null)
                it.remove(ACTIVATING_TUNNEL)
            else
                it[ACTIVATING_TUNNEL] = activatingTunnel
        }
    }
}
