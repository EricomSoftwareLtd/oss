...
class ObservableTunnel internal constructor(
    ...
    suspend fun setStateAsync(state: Tunnel.State): Tunnel.State = withContext(Dispatchers.Main.immediate) {
        ...
        if (state == this@ObservableTunnel.state) {
            return@withContext this@ObservableTunnel.state
        }

        if (this@ObservableTunnel.state == Tunnel.State.DOWN) {
                getConfigAsync().let { config ->
                    if (config.activationData.host != null && config.activationData.publicKey != null &&
                        config.activationData.host.toString() != "" && config.activationData.publicKey.toBase64() != ""
                    ) {
                        manager.activateTunnel(this@ObservableTunnel, config.activationData)
